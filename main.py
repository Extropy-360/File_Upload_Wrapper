...
class Portal:
    '''
        This class is used to interact with the client file portal. User will be logged in on class initiallization.

        Attributes:
            base_url (str): The base url of the client file portal.
        ...
        Example:
            >>> portal_client = Portal('https://files.portalsite.com', 'your_username', 'your_password')
            >>> portal_client.upload(['/path/to/file.txt', 'relative_path.csv'], 'path/on/portal')
    '''
    ...
    def _login(self):
        '''
                An internal method to authenticate the user with the Portal.
    ...
    def upload(self, file_paths: List[str], upload_to_path: 'str'):
        '''
            Uploads files to a specified path on the client file portal.

            Args:
                file_paths (list of str): A list of local file paths to upload.
                upload_to_path (str): The destination path on the client file portal, you can get this by navigating to the folder on the portal and copying the path from the side bar.
            ...
            Example:
                ...
                >>> base_url = 'https://files.fake_portal.com'
                >>> portal_client = Portal(base_url, 'your_username', 'your_password')
                >>> status = portal_client.upload(file_paths=files_to_upload, upload_to_path=upload_path)
